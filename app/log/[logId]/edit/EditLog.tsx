...
export default function EditLog({ logId }: { logId: string }) {
  ...
  useEffect(() => {
    if (logQuery.data) {
      if (logQuery.data.content) {
        setLogDescription(logQuery.data.content);
      }
      if (logQuery.data.image_url) {
        setSelectedImage(logQuery.data.image_url);
      }
      if (logQuery.data.coffee_name) {
        setCoffeeName(logQuery.data.coffee_name);
      }
      if (logQuery.data.coffee_place) {
        setCoffeePlace(logQuery.data.coffee_place);
      }
      if (logQuery.data.tags) {
        setCoffeeTags(logQuery.data.tags);
      }
      if (logQuery.data.recipe_id) {
        setSelectedRecipe({
          label: logQuery.data.recipes?.recipe_name,
          value: logQuery.data.recipe_id,
        });
      }
    }
  }, [logQuery.data]);
